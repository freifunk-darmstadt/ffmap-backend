...
class RRD(object):
            ...
            if b'<!-- Round Robin Archives -->' in line:
                ...
                for ds in new_ds:
                    restore.stdin.write(bytes("""
                        ...
                        """ % (ds.name, ds.type,
                               ds.args[0], ds.args[1], ds.args[2],
                               ds.last_ds, ds.value, ds.unknown_sec), 'utf-8'))
    ...
    def update(self, val):
        """
        Update the RRD with new values val.

        val can be either list or dict:
        ...
        * If it is a list, no template is generated and the order of the
        ...
        """
        try:
            args = ['N:' + ':'.join(map(str, val.values()))]
            kwargs = {'template': ':'.join(val.keys())}
        except AttributeError:
            args = ['N:' + ':'.join(map(str, val))]
            kwargs = {}
